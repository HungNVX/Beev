...
<div id="top-bar"></div>
            ...
            <nav class="">
                    <div class="container-fluid">
                        ...
                        <input class="form-control me-2" name ="textsearch" placeholder="Tìm kiếm bài viết ở đây">
                        <button class="btn btn-outline-success" name="timkiem">Search</button>
                        </form>
                        <?php require_once 'timkiembaiviet.php'?>
                    ...
                    <?php
                    //hiện post
                        $sql =($conn -> query("SELECT * FROM post ORDER BY idpost DESC LIMIT 10"));
                        $namecmt = $_SESSION['username'];
                        while($row =mysqli_fetch_array($sql)){
                            $idpost = $row[0];
                            $iduser = $row[1];
                            $get = ($conn->query("SELECT * FROM member WHERE id ='$iduser'"))->fetch_array();
                            $getname = $get[1];
                            $get= ($conn -> query("SELECT * FROM post WHERE idpost ='$idpost'"))->fetch_assoc();
                            $getnoidung = $get['noidung'];
                           ?>
                            <div class="card" style="width: 80rem;">
                                <div class="card-body">
                                    <h5 class="card-title" style ="font-size:42px">-<?= $getname ?>-</h5>
                                    <h8 class="card-title"><?= $get['date']?></h8>
                                    <p class="card-text" style ="font-size:22px"><?= $getnoidung ?></p> 
                                    <?php //hiện cmt 
                                            $getcmt = ($conn -> query("SELECT * FROM comment WHERE idpost ='$idpost'"));
                                        while($roww =mysqli_fetch_assoc($getcmt)){
                                            $getid = $roww['id'];
                                            $getuser = ($conn-> query("SELECT * FROM member WHERE id='$getid'"))->fetch_assoc();
                        
                                     ?>
                                    <p style ="font-size:12px">-<?=$getuser['username']?>- : <?=$roww['text'] ?></p> 
                                    <?php } ?>
                                    <div class="container-fluid">
